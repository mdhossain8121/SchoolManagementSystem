    ...
    public partial class frmTeacherAttendance : Form
    {
        ...
        private void checkFinger()
        {
            try
            {
                ...
                DataTable finger = aTeacherManager.GetAllFingerData(aTeacher);
                ...
                foreach (DataRow dr in finger.Rows)
                {
                    ...
                    if (err == (Int32)SGFPMError.ERROR_NONE)
                    {
                        if (finger_found)
                        {
                            ...
                            attendance.PersonType = 2; //1 for student 2 for teacher
                            attendance.TodayDate = DateTime.Now;
                            if (attendance.Insert())
                            {
                                StatusBar.Text = "Data Matched - " + dr["TEACHER_NAME"].ToString();
                            }
                            else
                            {
